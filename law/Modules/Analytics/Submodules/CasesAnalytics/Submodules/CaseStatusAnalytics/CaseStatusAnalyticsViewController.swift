//
//  CaseStatusAnalyticsViewController.swift
//  law
//
...
final class CaseStatusAnalyticsViewController: BaseViewController {
    ...
    private let viewModel: CaseStatusAnalyticsViewModelProtocol

	init(viewModel: CaseStatusAnalyticsViewModelProtocol) {
    ...
    override func setupBindings() {
        ...
        self.viewModel.xAxisPublished.sink { [weak self] xAxis in
            self?.barChartView.xAxis.valueFormatter = xAxis
        }.store(in: &cancellables)
    }
    
    override func setupNavigationController() {
        self.navigationItem.title = "Дела по статусам"
    }
}
